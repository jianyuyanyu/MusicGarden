...
namespace MusicGarden.Http
{
    public class WebClient
    {
        ...
        private void WriteFileField(string fileField)
        {
            ...
            {
                Match M = Regex.Match(var, "([^=]+)=(.+)");
                string filePath2 = M.Groups[2].Value;
                fileField = "----HEDAODE--\r\n";
        ...
        private void SendRequestData(string URL, string method, bool showProgress)
        {
            ...
            while (!isCanceled)
            {
                int count2 = postStream.Read(buffer, 0, buffer.Length);
        ...
        private byte[] GetRequestHeaders(string request)
        {
            ...
            requestHeaders.Add("User-Agent", "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.77 Safari/537.36");
        ...
        private void SaveNetworkStream(Stream toStream, bool showProgress)
        {
            ...
            GetResponseHeader(buffer2, out int startIndex);
            count4 -= startIndex;
